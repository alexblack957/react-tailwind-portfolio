...
const AnimateOnScroll = ({ children, reappear, threshold = 0.1 }: Props) => {
  ...
  return (
    <>
      <div ref={containerRef} className={`transition duration-700 ${isVisible ? "opacity-100 blur-none translate-x-0" : "opacity-0 blur-xs translate-y-10"}  motion-reduce:transition-none motion-reduce:hover:transform-none`}>
        {children}
      </div>
    </>
  );
